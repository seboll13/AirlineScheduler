...
def __compute_composite_score(distance, **kwargs) -> float:
    ...
    return (
        WEIGHT_PF * __compute_population_factor(
            populations[:2], populations[3]
        ) +
        WEIGHT_EF * __compute_economic_factor(
            gdps[:2], plis[:2]
        ) +
        WEIGHT_TF * __compute_tourism_factor(
            tourism_expenditures[:2], tourism_expenditures[3]
        ) +
...
def _compute_base_demand(distance, **kwargs) -> float:
    ...
    """
    return __compute_composite_score(
        distance, **kwargs
...
# --------------------------------------------------------------------- #
def _get_first_class_multiplier(
    gdps: tuple[float, float],
    plis: tuple[float, float]
):
    ...
    """
    return __compute_economic_factor(gdps, plis) * 0.02
...
def _get_business_class_multiplier(
    ...
    """
    return __compute_economic_factor(
        gdps, plis
    ) * 0.05 + __compute_tourism_factor(
        tourism_expenditures, product_of_expenditures
    ) * 0.03
...
def _get_economy_class_multiplier(
    ...
    """
    return (__compute_population_factor(
        populations, product_of_populations
    ) + __compute_distance_factor(distance)) * 0.8
